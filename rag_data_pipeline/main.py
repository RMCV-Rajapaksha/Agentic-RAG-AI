...
def process_youtube_videos(urls: List[str], segment_length_minutes: int = 10) -> List[Document]:
...
def filter_duplicate_documents(documents: List[Document], vector_store) -> List[Document]:
    ...
    try:
        existing_urls = set()
        try:
            all_nodes = vector_store._get_all() if hasattr(vector_store, '_get_all') else []
...
def ingest_documents(documents: List[Document], pipeline, vector_store) -> None:
